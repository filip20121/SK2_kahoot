  ...
  while (1) {
    ...
    if (sign == '@') {
      ...
      if (index == -1) {
        ...
        message = "Rozpocząłeś quiz. Poczekaj aż się zakończy.";
        sendQuestion(clientFd, message);
        ...
        if (clientFd == quiz -> authorFd || quiz -> accessCode == 0 || quiz -> accessCode == 1) {
          ...
          //allow to enter the quiz until the end of time for joining
          quiz -> lock = 0;
          ...
          //block entering and enable sending questions 
          quiz -> lock = 2;
      ...
      if (index == -1) {
        ...
        if (clientFd == quiz -> authorFd) { //ptr_tab[index]
          ...
          if (quiz -> lock == 0) {
            ...
            message = "Dołączyłeś do quizu. \nQuiz rozpocznie się w przeciągu " + to_string(time4ans) + " sekund.\n";
            sendQuestion(clientFd, message);
